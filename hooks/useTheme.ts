...
function applyPreference(nextPref: ThemePreference, origin?: ToggleOrigin): void {
...
export function useTheme() {
  ...
  const toggleTheme = useCallback((origin?: ToggleOrigin) => {
    const current = ensureState();
    applyPreference(nextPreference(current.preference), origin);
  }, []);

  // Directly select a preference (settings UI) instead of cycling through them.
  const setPreference = useCallback((pref: ThemePreference, origin?: ToggleOrigin) => {
    applyPreference(pref, origin);
  }, []);
