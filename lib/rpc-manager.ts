...
export class AgentSessionWrapper {
  ...
  start(): void {
    this.unsubscribe = this.inner.subscribe((event: AgentEvent) => {
      ...
      if (event.type === "message_end") {
        // The SDK writes the session file right after dispatching message_end
        // (appendMessage → _persist), so a brand-new session only becomes
        // visible on disk once its first assistant message completes. Queue a
        // microtask so that write has happened before we check, then invalidate
        // the list cache — otherwise /api/sessions would keep serving the stale
        // 30s cache and the sidebar would never pick the new session up mid-run.
        queueMicrotask(() => {
          const file = this.inner.sessionFile;
          if (file && existsSync(file)) invalidateSessionListCache();
        });
      }
    ...
    this.extensionBindingPromise = (async () => {
      ...
      if (typeof this.inner.bindExtensions === "function") {
        ...
        await bindExtensions.call(this.inner, {
          ...
          onError: (error) => {
            // Stale-ctx noise: after wrapper teardown (idle timeout / destroy /
            // hot reload), extension background tasks (timers, watchers, IPC
            // channels) keep firing on the invalidated runtime. This is not
            // actionable by the user — log server-side with the event name so
            // the culprit can be traced, but do not surface a popup.
            if (/extension ctx is stale/i.test(error.error)) {
              console.warn(
                `[pi-web] stale extension handler suppressed: ${error.extensionPath} event=${error.event}`,
              );
              return;
            }
            this.emit({
  ...
  destroy(): void {
    ...
    // Give extensions a chance to tear down background tasks (timers, file
    // watchers, IPC channels) before the SDK invalidates their runtime.
    // shutdown() already emitted this; destroy() paths that bypass it
    // (process exit, direct destroy) fire it here. destroy() is synchronous,
    // so this is fire-and-forget: handler synchronous prefixes (clearTimeout,
    // stop(), dispose() calls) run before dispose() invalidates the runtime;
    // async continuations are covered by the stale-ctx error filter.
    if (!this.shutdownPromise) {
      try {
        void this.inner.extensionRunner?.emit?.({ type: "session_shutdown", reason: "quit" });
      } catch {
        // Best effort: extensions may not be bound yet.
      }
    }
    try {
      this.inner.dispose();
...
declare global {
  ...
  var __piAgentServicesCache: Map<string, { promise: Promise<AgentSessionServices>; expiresAt: number }> | undefined;
}
...
const AGENT_SERVICES_TTL_MS = 5 * 60 * 1000;

/**
 * createAgentSessionServices() is expensive: it jiti-transpiles every extension
 * (the SDK clears its extension cache when the cwd changes) and refreshes model
 * auth. startRpcSession() and GET /api/models both need it and a session switch
 * fires them at the same time, so share one build per cwd. Stored on globalThis
 * (hot-reload safe, same pattern as __piSessions).
 */
export async function getOrCreateAgentSessionServices(
  cwd: string,
  trustReloadOptions: { resolveProjectTrust: () => Promise<boolean> } | undefined,
): Promise<AgentSessionServices> {
  if (!globalThis.__piAgentServicesCache) globalThis.__piAgentServicesCache = new Map();
  const cache = globalThis.__piAgentServicesCache;
  const cached = cache.get(cwd);
  if (cached && cached.expiresAt > Date.now()) return cached.promise;

  const promise = createAgentSessionServices({
    cwd,
    agentDir: getAgentDir(),
    ...(trustReloadOptions ? { resourceLoaderReloadOptions: trustReloadOptions } : {}),
  });
  cache.set(cwd, { promise, expiresAt: Date.now() + AGENT_SERVICES_TTL_MS });
  // Never cache a rejected build — let the next caller retry.
  promise.catch(() => {
...
/**
 * Drop all cached agent services. Call whenever models.json, provider auth, or
 * project trust is rewritten out from under live services objects.
 */
export function invalidateAgentServicesCache(): void {
  globalThis.__piAgentServicesCache = undefined;
}
...
export async function startRpcSession(
  ...
  const starting = (async () => {
    ...
    const services = await getOrCreateAgentSessionServices(sessionCwd, trustReloadOptions);
