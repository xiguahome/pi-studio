...
/** One structured question parsed from the ask_user_question tool input. */
...
/**
 * User's answer to one questionnaire question.
 * - option: picked option at `optionIndex` (0-based) of the question's options.
 * - custom: typed a free-text answer via the "Type something." row (single-select).
 * - multi: multiSelect — picked `indices` and/or typed `customText`.
 */
export type QuestionnaireAnswer =
  | { kind: "option"; optionIndex: number }
  | { kind: "custom"; text: string }
  | { kind: "multi"; indices: number[]; customText?: string };
...
export interface SessionInfo {
  ...
  /** True when this session is a subagent child session (stored in a nested run directory). */
  isSubagent?: boolean;
  /** For subagent sessions: the agent name (e.g. "worker") shown in the sidebar. */
  subagentName?: string;
}
