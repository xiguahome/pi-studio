...
export function getGlobalSkillsLockPath({
  ...
  return xdgStateHome
    ...
    : join(homeDir, ".pi-studio", ".skill-lock.json");
}
...
interface RemoveLockEntryOptions {
  cwd: string;
  /** Accepted for caller symmetry; not currently used by the resolver. */
  agentDir?: string;
  /** Override the global lock path (defaults to ~/.pi-studio/.skill-lock.json). */
  globalLockPath?: string;
  projectLockPath?: string;
}

/**
 * Remove a skill's entry from its scope's lock file (global or project).
 * No-op if the lock file or entry is absent. Key matching is case-insensitive
 * (mirrors findLockEntry) and writes back atomically. Used when uninstalling a
 * skill that was installed via `npx skills add`.
 */
export function removeSkillLockEntry(
  skillName: string,
  scope: SkillInstallScope,
  {
    cwd,
    globalLockPath = getGlobalSkillsLockPath(),
    projectLockPath = join(cwd, "skills-lock.json"),
  }: RemoveLockEntryOptions,
): void {
  const lockPath = scope === "global" ? globalLockPath : projectLockPath;
  if (!existsSync(lockPath)) return;
  let parsed: SkillLockFile;
  try {
    parsed = JSON.parse(readFileSync(lockPath, "utf8")) as SkillLockFile;
  } catch {
    return;
  }
  ...
  if (Object.prototype.hasOwnProperty.call(parsed.skills, skillName)) {
    key = skillName;
  } else {
    const lower = skillName.toLowerCase();
    key = Object.keys(parsed.skills).find((k) => k.toLowerCase() === lower);
  }
  if (!key) return;

  delete parsed.skills[key];
  writeFileAtomicSync(lockPath, JSON.stringify(parsed, null, 2));
}
