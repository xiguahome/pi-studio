...
export function buildSkillUpdateArgs(install: SkillInstallInfo): string[] {
  ...
  const args = [
    ...
    "-y",
    // "cline" maps to ~/.pi-studio/skills + cwd/.pi-studio/skills; "--agent pi"
    // would write into the pi CLI's isolated ~/.pi/agent/skills instead.
    "--agent",
    "cline",
  ];
...
async function resolveGitTreeHash(install: SkillInstallInfo): Promise<string> {
  ...
  try {
    ...
    await execFileAsync("git", [
      ...
      repository,
      ref,
    ], { timeout: GIT_CHECK_TIMEOUT_MS, env: withProxyEnv({ ...process.env }) });
...
export async function checkSkillUpdate(
  ...
  const resolvedOptions = {
    ...options,
    // Default to the proxy-aware fetch so api.github.com / skills.sh honor
    // the configured proxy (Node's global fetch ignores HTTP_PROXY).
    fetcher: options.fetcher ?? ((input, init) => proxiedFetch(input, init)),
...
export async function checkSkillUpdates(
  ...
  const fetcher = options.fetcher ?? ((input, init) => proxiedFetch(input, init));
