...
import {
  ensureBuiltinNpmDirMirrors,
  ensureSdkNpmCommand,
  isBuiltinSource,
  readBuiltinSeedStatus,
  runPostInstallPatches,
} from "@/lib/builtin-extensions";
...
async function readPlugins(cwd: string): Promise<PluginsResponse> {
  ...
  const packages = packageManager.listConfiguredPackages().map((pkg) => {
    ...
    return {
      ...
      disabled,
      builtin: isBuiltinSource(pkg.source),
  ...
  return {
    ...
    seedStatus: readBuiltinSeedStatus() ?? undefined,
  };
}
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Idempotent: make sure the SDK's installs use the bundled runtime npm
    // (boot already does this, but the panel may act before/without a seed).
    ...
    if (body.action === "install") {
      ...
      // Ensure the managed npm dir's .npmrc carries the npmmirror binary
      // mirrors (better-sqlite3 etc.) before the SDK's DefaultPackageManager
      // runs npm. Without them prebuild-install would fetch from GitHub
      // (unreachable without a proxy from mainland CN) and fall back to a
      // node-gyp compile that needs VS Build Tools — both fail out of the box.
      await ensureBuiltinNpmDirMirrors();
      ...
      if (isBuiltinSource(source)) {
        return NextResponse.json(
          { error: "Built-in extensions cannot be removed", builtin: true },
          { status: 403 },
        );
      }
      await packageManager.removeAndPersist(source, { local });
    } else if (body.action === "update") {
      if (source && isBuiltinSource(source)) {
        // Built-ins are pinned to the pi-studio release — "update" would just
        // reinstall the same pinned spec. Keep the API symmetric with remove.
        return NextResponse.json(
          { error: "Built-in extensions are updated with pi-studio releases", builtin: true },
          { status: 403 },
        );
      }
