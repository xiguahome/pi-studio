...
type ExportHtmlModule = {
  exportFromFile: (
    inputPath: string,
    options: string | { outputPath: string; themeName?: string }
  ) => Promise<string>;
};
...
/**
 * Resolve the pi-coding-agent export-html module at runtime via a native
 * (Node) dynamic import.
 *
 * The path is computed at runtime from the package directory, so it cannot be
 * statically analyzed by webpack. We mark the import with `webpackIgnore: true`
 * to keep it as a native `import()` that Node resolves at runtime against the
 * absolute `file://` URL — otherwise the bundled API route throws
 * "Cannot find module as expression is too dynamic".
 */
async function loadExportFromFile(packageDir: string): Promise<ExportHtmlModule> {
  const exporterUrl = pathToFileURL(
    join(packageDir, "dist", "core", "export-html", "index.js")
  ).href;
  return (await import(/* webpackIgnore: true */ exporterUrl)) as ExportHtmlModule;
}
...
async function exportSession(
  ...
  // A themed export must use the in-process generator, which honors `themeName`.
  // The pi CLI's `--export` path ignores the theme and always falls back to dark
  // (see pi-coding-agent main.js: `exportFromFile(parsed.export, outputPath)` with
  // no themeName, which then resolves to the default dark theme on the server).
  // So for any explicit theme we bypass the CLI and call the exporter directly.
  if (themeName && packageDir) {
    const { exportFromFile } = await loadExportFromFile(packageDir);
    await exportFromFile(filePath, { outputPath, themeName });
    return;
  }
...
export async function GET(
  ...
  const themeName: "light" | "dark" | undefined =
  ...
  try {
    ...
    try {
      await exportSession(filePath, outputPath, themeName);
      ...
      // 嵌入模式允许 iframe 展示，下载模式禁止
      if (inline) {
        // 允许同源 iframe 嵌入
        headers["Content-Security-Policy"] = "frame-ancestors 'self'";
        // 移除 X-Frame-Options 禁止头（已被 CSP 替代）
      } else {
        headers["Content-Security-Policy"] = "frame-ancestors 'none'";
        headers["X-Frame-Options"] = "DENY";
      }

      return new Response(patchedHtml, { headers });
    } finally {
