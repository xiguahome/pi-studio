...
// skill paths, package skills, and .pi-studio/skills directories are all included.
...
export async function PATCH(req: Request) {
  try {
    ...
    // Built-in skills may still be toggled: the edit writes the
    // disable-model-invocation flag into the seeded SKILL.md, which the
    // boot-time seed treats as a user modification and never overwrites.
    const { allowed } = await checkSkillPathAccess(filePath);
    if (!allowed) {
      return NextResponse.json({ error: "Access denied" }, { status: 403 });
    }
