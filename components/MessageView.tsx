...
export const MessageView = memo(function MessageView({ message, isStreaming, toolResults, modelNames, cwd, onOpenFile, onOpenBrowser, entryId, onFork, forking, onNavigate, prevAssistantEntryId, onEditContent, showTimestamp, prevTimestamp, sessionId, writtenFiles }: Props) {
  if (message.role === "user") {
    return <UserMessageView message={message as UserMessage} cwd={cwd} onOpenFile={onOpenFile} entryId={entryId} onFork={onFork} forking={forking} onNavigate={onNavigate} prevAssistantEntryId={prevAssistantEntryId} onEditContent={onEditContent} />;
  }
  if (message.role === "assistant") {
    return <AssistantMessageView message={message as AssistantMessage} isStreaming={isStreaming} toolResults={toolResults} modelNames={modelNames} cwd={cwd} onOpenFile={onOpenFile} onOpenBrowser={onOpenBrowser} showTimestamp={showTimestamp} prevTimestamp={prevTimestamp} sessionId={sessionId} entryId={entryId} writtenFiles={writtenFiles} />;
  }
  if (message.role === "toolResult") {
    // plan_mode_complete carries the full proposed plan as its result text —
    // surface it as a prominent inline block instead of burying it in the
    // collapsed process details under the tool call.
    const toolName = (message as ToolResultMessage).toolName;
    if (toolName === "plan_mode_complete") {
      return <PlanCompleteView message={message as ToolResultMessage} cwd={cwd} onOpenFile={onOpenFile} />;
    }
    // goal_complete / goal_blocked likewise carry the goal outcome as result
    // text — render them as standalone success/blocked cards.
    if (toolName === "goal_complete" || toolName === "goal_blocked") {
      return <GoalCompleteView message={message as ToolResultMessage} cwd={cwd} onOpenFile={onOpenFile} />;
    }
  ...
  return prev.message === next.message
    ...
    && prev.onOpenBrowser === next.onOpenBrowser
    ...
    >
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        {blockItems.map(({ block, originalIndex }) => (
          <BlockView key={`${entryId ?? "stream"}-${originalIndex}`} block={block} toolResults={toolResults} isStreaming={isStreaming} streamingDuration={streamingDurations.get(originalIndex) ?? (block.type === "thinking" ? thinkingDurationFromFile : undefined)} toolCallDurations={toolCallDurations} cwd={cwd} onOpenFile={onOpenFile} onOpenBrowser={onOpenBrowser} sessionId={sessionId} entryId={entryId} blockIndex={originalIndex} />
        ))}
      </div>
...
function BlockView({ block, toolResults, isStreaming, streamingDuration, toolCallDurations, cwd, onOpenFile, onOpenBrowser, sessionId, entryId, blockIndex }: { block: AssistantContentBlock; toolResults?: Map<string, ToolResultMessage>; isStreaming?: boolean; streamingDuration?: number; toolCallDurations?: Map<string, number>; cwd?: string; onOpenFile?: (filePath: string) => void; onOpenBrowser?: (options: { url?: string; htmlContent?: string; label?: string }) => void; sessionId?: string; entryId?: string; blockIndex: number }) {
  if (block.type === "text") {
    return <TextBlock block={block as TextContent} isStreaming={isStreaming} cwd={cwd} onOpenFile={onOpenFile} onOpenBrowser={onOpenBrowser} />;
  }
  ...
  if (block.type === "toolCall") {
    ...
    // plan_mode_complete / goal_complete / goal_blocked surface their result
    // text as standalone cards — don't also render it inside the tool-call block.
    const isStandaloneCard = tc.toolName === "plan_mode_complete" || tc.toolName === "goal_complete" || tc.toolName === "goal_blocked";
    return <ToolCallBlock block={tc} result={isStandaloneCard ? undefined : result} duration={duration} />;
  }
  return null;
}

function TextBlock({ block, isStreaming, cwd, onOpenFile, onOpenBrowser }: { block: TextContent; isStreaming?: boolean; cwd?: string; onOpenFile?: (filePath: string) => void; onOpenBrowser?: (options: { url?: string; htmlContent?: string; label?: string }) => void }) {
  return <SafeMarkdownBody isStreaming={isStreaming} cwd={cwd} onOpenFile={onOpenFile} onOpenBrowser={onOpenBrowser}>{block.text}</SafeMarkdownBody>;
}
...
function PlanCompleteView({ message, cwd, onOpenFile }: { message: ToolResultMessage; cwd?: string; onOpenFile?: (filePath: string) => void }) {
  const raw = typeof message.content === "string"
    ...
    : Array.isArray(message.content)
      ? message.content
          ...
          .join("\n")
      : "";
  // pi-plan-mode prefixes the plan with "**Proposed Plan**"; drop it since the
  // card already carries its own title.
  const planText = raw.replace(/^\s*\*\*Proposed Plan\*\*\s*\n*/i, "");
  return (
    <div style={{
      margin: "12px 0",
      ...
      <div style={{
        ...
        <span style={{ fontSize: 14 }}>📋</span> 实施方案
      </div>
      <div style={{ padding: "4px 14px 10px", fontSize: 14 }}>
        <SafeMarkdownBody cwd={cwd} onOpenFile={onOpenFile}>{planText}</SafeMarkdownBody>
      </div>
    </div>
  );
}

function GoalCompleteView({ message, cwd, onOpenFile }: { message: ToolResultMessage; cwd?: string; onOpenFile?: (filePath: string) => void }) {
  const raw = typeof message.content === "string"
    ? message.content
    : Array.isArray(message.content)
      ? message.content
          ...
          .join("\n")
      : "";
  const blocked = message.toolName === "goal_blocked";
  // pi-goal prefixes the result with "Goal complete: " / "Goal blocked: ";
  // drop it since the card already carries its own title.
  let text = raw.replace(blocked ? /^\s*Goal blocked:\s*/i : /^\s*Goal complete:\s*/i, "");
  // A queued goal (if any) is appended as "Next goal queued: <text>" — surface
  // it as a muted footer line instead of burying it in the summary markdown.
  let queuedGoal: string | null = null;
  const queuedMatch = text.match(/\nNext goal queued: (.+)$/i);
  if (queuedMatch?.[1]) {
    queuedGoal = queuedMatch[1].trim();
    text = text.slice(0, queuedMatch.index);
  }
  const border = blocked ? "rgba(245,158,11,0.4)" : "rgba(34,197,94,0.4)";
  const background = blocked ? "rgba(245,158,11,0.05)" : "rgba(34,197,94,0.05)";
  const headerBorder = blocked ? "rgba(245,158,11,0.25)" : "rgba(34,197,94,0.25)";
  const headerColor = blocked ? "rgba(180,130,0,1)" : "rgba(21,128,61,1)";
  return (
    <div style={{
      margin: "12px 0",
      ...
      <div style={{
        ...
        <span style={{ fontSize: 14 }}>{blocked ? "⚠️" : "✅"}</span> {blocked ? "目标受阻" : "目标完成"}
      </div>
      <div style={{ padding: "4px 14px 10px", fontSize: 14 }}>
