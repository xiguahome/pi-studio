...
function SkillDetail({
  ...
  // ── Inline SKILL.md editor (non-builtin skills only) ──
  ...
  const dirty = editing && editContent !== originalContent;
  ...
  const loadContent = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/skills/content?filePath=${encodeURIComponent(skill.filePath)}`,
      );
  ...
  const saveContent = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || d.error || !d.success) {
        if (res.status === 409) {
          setConflict(true);
          setEditError(d.error ?? t("i18n.editConflict"));
        } else {
  ...
  }, [skill.filePath, editContent, baseMtime, onSaved, t]);

  const cancelEdit = useCallback(() => {
    if (dirty && !window.confirm(t("i18n.discardChanges"))) return;
    setEditing(false);
    setEditError(null);
    setConflict(false);
  }, [dirty, t]);
          ...
          {canCheckAll && !skill.builtin && (
            ...
            >
              {checkingAll
                ? t("i18n.checking")
                : pendingUpdates > 0
                  ? `${t("i18n.checkUpdates")} (${pendingUpdates})`
                  : t("i18n.checkUpdates")}
            </button>
          )}
          {!skill.builtin && (
            ...
            >
              {loadingContent ? t("i18n.loading") : t("i18n.editSkill")}
            </button>
          )}
          {!skill.builtin && skill.sourceInfo.origin !== "package" && (
            <button
            ...
            >
              {deleting ? t("i18n.loading") : t("i18n.deleteSkill")}
            </button>
          )}
          {!skill.builtin && skill.sourceInfo.origin === "package" && (
            <span style={{ fontSize: 11, color: "var(--text-dim)", flexShrink: 0 }}>
              {t("i18n.packageSkillHint")}
            </span>
          )}
          <Toggle
            enabled={enabled}
...
function AddSkillPanel({
  ...
  const installPath =
    scope === "global"
      ? "~/.pi-studio/skills/"
      : `${shortenPath(cwd)}/.pi-studio/skills/`;
...
export function SkillsConfig({
  ...
  // True while the SKILL.md editor holds unsaved changes; guards navigation.
  const [editorDirty, setEditorDirty] = useState(false);
  // filePaths currently being deleted (disables the delete button + shows loading).
  ...
  const deleteSkill = useCallback(async (skill: Skill) => {
    if (!window.confirm(t("i18n.deleteSkillConfirm", { name: skill.name }))) return;
    ...
    try {
      const res = await fetch("/api/skills/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ filePath: skill.filePath, cwd }),
      });
      const d = (await res.json()) as { success?: boolean; error?: string };
      if (!res.ok || d.error) {
        setSaveError(d.error ?? `HTTP ${res.status}`);
      ...
      setSelected(first?.filePath ?? null);
    } catch (e) {
      setSaveError(String(e));
    } finally {
      setDeleting((s) => {
  ...
  const confirmDiscardIfDirty = useCallback((): boolean => {
    ...
    return window.confirm(t("i18n.discardChanges"));
  }, [editorDirty, t]);
              ...
              {loading ? (
                ...
                (() => {
                  ...
                  const groupDefinitions = [
                    {
                      label: "builtin",
                      matches: (skill: Skill) => Boolean(skill.builtin),
                    },
                    {
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
            {addMode ? (
              ...
              <SkillDetail
                ...
                canCheckAll={skills.some((skill) => Boolean(skill.install))}
                ...
                onCheckAll={() => void checkForUpdates()}
              />
            ) : (
              <div
                style={{
