...
// Client-side TTL caches keyed by cwd: switching folders or sessions reuses the
// last git status / root listing instead of re-running git subprocesses and
// directory scans. Explicit refreshes (refreshKey / treeRefreshKey) bypass them.
const gitStatusCache = createClientTtlCache<GitStatusResponse>(15_000);
const fileListCache = createClientTtlCache<FileNode[]>(15_000);
...
export const FileExplorer = forwardRef<FileExplorerHandle, Props>(function FileExplorer({
  ...
  const prevGitCwdRef = useRef<string | null>(null);
  ...
  useEffect(() => {
    ...
    // Cache hit on cwd change (returning to a recently-seen folder skips the
    // network round trip). Explicit refreshKey / treeRefreshKey bumps bypass.
    const cached = cwdChanged ? fileListCache.get(cwd) : undefined;
    if (cached) {
      setRoots(cached);
      setError(null);
      setLoading(false);
  ...
  useEffect(() => {
    const cwdChanged = prevGitCwdRef.current !== cwd;
    prevGitCwdRef.current = cwd;
    const cached = cwdChanged ? gitStatusCache.get(cwd) : undefined;
    if (cached) {
      setGitFiles(cached.isGitRepository ? cached.files : []);
      setGitLineStats(cached.isGitRepository
        ? { additions: cached.additions, deletions: cached.deletions }
