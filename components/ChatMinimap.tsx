...
export function ChatMinimap({
  ...
  const containerHeight = useMemo(() => {
    const idealHeight = MINIMAP_PADDING * 2 + Math.max(0, allNodes.length - 1) * MAX_NODE_GAP;
    return Math.min(idealHeight, availableHeight);
  }, [allNodes.length, availableHeight]);
  const nodeLayout = useMemo(
    () => layoutNodes(allNodes, containerHeight),
    [allNodes, containerHeight],
  );
  ...
  const measureNodes = useCallback(() => {
    ...
    measureThrottleRef.current = setTimeout(() => {
      ...
      setAvailableHeight(wrapperEl.clientHeight);
  ...
  // Vertical position of the hovered node within the wrapper. The inner
  // minimap bar is centered in the wrapper, so start from the centered
  // offset, then add the node's ratio * bar height.
  const containerOffsetTop = (availableHeight - containerHeight) / 2;
  const previewAnchorY = nearestNode
    ? containerOffsetTop + nearestNode.topRatio * containerHeight
    : 0;

  if (!visible) return null;

  const lastNodeTop = positionedNodes.length > 0
    ? positionedNodes[positionedNodes.length - 1].topRatio * containerHeight
    : MINIMAP_PADDING;
    ...
    >
            ...
            <div
              style={{
                ...
                boxShadow: isActive
                  ...
                  : "none",
                transition: "transform 0.1s, background 0.14s ease, border-color 0.14s ease, box-shadow 0.14s ease",
      ...
      {minimapHovered && nearestNode && (
        <div
          ...
          style={
            previewAnchorY <= availableHeight / 2
              ? { top: previewAnchorY }
              : { bottom: availableHeight - previewAnchorY }
          }
          onMouseEnter={showPreview}
          onMouseLeave={schedulePreviewHide}
        ...
        >
          <div
            ...
            data-minimap-preview-index={nearestNode.index}
          >
            <span className={styles.number} aria-hidden="true">
              {String(nearestNode.index + 1).padStart(2, "0")}
            </span>
            <div className={styles.content}>
              <button
                type="button"
                className={styles.user}
                data-minimap-preview-user={nearestNode.index}
                onClick={() => {
                  scrollToNode(nearestNode, "smooth");
                }}
              >
                <span className={styles.userText}>
                  {getUserPreview(nearestNode.targetTurn.userMessage)}
                </span>
              </button>

              {nearestNode.targetTurn.assistantPreviews.map((assistant, assistantIndex) => (
                <div
                  key={assistantIndex}
                ...
                >
                  <button
                    ...
                    data-minimap-preview-assistant={`${nearestNode.index}-${assistantIndex}`}
                    onClick={() => scrollToAssistant(nearestNode, assistantIndex)}
                  ...
                  <AssistantOutline
                    ...
                    onAnswerClick={() => scrollToAssistant(nearestNode, assistantIndex)}
                    onHeadingClick={(headingIndex) => (
                      scrollToHeading(nearestNode, assistantIndex, headingIndex)
                    )}
                  />
                </div>
              ))}
