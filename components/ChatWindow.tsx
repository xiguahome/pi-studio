...
import { ProjectHeader, type BranchSwitchState, type ProjectInfo } from "./ProjectHeader";
...
interface Props {
  ...
  // Notified when the agent starts executing any tool, so AppShell can auto-open
  // the built-in browser on the first chrome-devtools tool call.
  onToolStart?: (toolName: string) => void;
  /** Project + git context for the floating chip above the chat content. */
  projectInfo?: ProjectInfo | null;
  onSwitchBranch?: (branch: string) => void;
  branchSwitchState?: BranchSwitchState;
  /** Create a new branch, optionally tracking a remote branch. */
  onCreateBranch?: (name: string, startPoint: string | null) => void;
  branchCreateState?: BranchSwitchState;
  /** Safely delete a non-current local branch. */
  onDeleteBranch?: (branch: string) => void;
  branchDeleteState?: BranchSwitchState;
  /** Open the git history tab in the right panel (from the project chip's branch menu). */
...
// Extra rightward offset for the floating project/git capsule chip.
const PROJECT_CAPSULE_EXTRA_LEFT = 24;

/** Fallback before /api/worktrees resolves: the chip still shows the folder. */
...
export function ChatWindow({ session, sessionRunning, newSessionCwd, newSessionDraftKey, onAgentEnd, onSessionCreated, onSessionForked, chatInputRef, onBranchDataChange, onSessionStatsChange, onSessionStatsPanelOpen, onContextUsageChange, onOpenFile, onOpenBrowser, onToolStart, projectInfo, onSwitchBranch, branchSwitchState, onCreateBranch, branchCreateState, onDeleteBranch, branchDeleteState, onShowGitHistory, soundEnabled = true, onSoundToggle, playDoneSound = () => {}, unlockAudio }: Props) {
  const { t } = useI18n();
  ...
  const {
    ...
    chatInputRef, onBranchDataChange, onSessionStatsPanelOpen, onToolStart,
  });
  ...
  const chatInputElement = (
    <ChatInput
      ...
      planModeMenu={planModeMenu}
      onPlanMenuRespond={session || isNew ? respondToPlanMenu : undefined}
      onPlanAction={session || isNew ? handlePlanAction : undefined}
  ...
  // Allow dismissing the plan-mode status widget via a close (✕) button —
  // closing it sends "/plan exit" to leave Plan mode.
  const widgetCloseProps = {
    closableKeys: ["plan-mode-plan"],
    onCloseWidget: (key: string) => {
      if (key === "plan-mode-plan") sendModePrompt("/plan exit");
    },
  };
  ...
  return (
    ...
    >
      ...
      {extensionDialog && (extensionDialog.method === "select" || extensionDialog.method === "input") ? (
        <AskUserQuestionCard
      ...
      {/* Project folder + git branch chip — always visible, both on the
       *  welcome screen and while a session is open. */}
      {onSwitchBranch && (
        <ProjectHeader
          info={projectInfo ?? EMPTY_PROJECT_INFO}
          cwd={session?.cwd ?? newSessionCwd ?? null}
          onSwitch={onSwitchBranch}
          switchState={branchSwitchState}
          onCreateBranch={onCreateBranch}
          createState={branchCreateState}
          onDeleteBranch={onDeleteBranch}
          deleteState={branchDeleteState}
          offsetLeft={isMobile ? 8 + 12 : CHAT_COLUMN_PADDING + PROJECT_CAPSULE_EXTRA_LEFT}
          sessionId={session?.id ?? null}
          onOpenGitHistory={onShowGitHistory}
        />
      )}
      ...
      {isEmptyNew ? (
        <div className="flex flex-1 flex-col items-center justify-center overflow-y-auto px-4 py-8">
          <div className="w-full max-w-[820px]">
            ...
            <ExtensionWidgets widgets={aboveEditorWidgets} {...widgetCloseProps} />
            {chatInputElement}
            <ExtensionWidgets widgets={belowEditorWidgets} {...widgetCloseProps} />
          </div>
        </div>
      ) : (
      <>
        ...
        <div ref={scrollContainerRef} className="min-w-0 flex-1 overflow-x-hidden overflow-y-auto pt-10">
          <div style={{ minWidth: 0, padding: `0 ${CHAT_COLUMN_PADDING}px` }}>
            ...
            {(() => {
              ...
              for (let idx = 0; idx < messages.length;) {
                ...
                const planCompleteIndices: number[] = [];
                for (let processIdx = userIdx + 1; processIdx < finalAssistantIdx; processIdx++) {
                  const pm = messages[processIdx];
                  // plan_mode_complete / goal_complete / goal_blocked render as
                  // their own prominent cards — keep them out of the collapsed
                  // process group.
                  const cardToolName = (pm as { toolName?: string }).toolName;
                  if (pm?.role === "toolResult" && (cardToolName === "plan_mode_complete" || cardToolName === "goal_complete" || cardToolName === "goal_blocked")) {
                    planCompleteIndices.push(processIdx);
                  } else {
                ...
                // Plan cards stay visible (not folded into process details).
                for (const planIdx of planCompleteIndices) {
                  rendered.push(renderMessage(planIdx, { attachRef: false, keyPrefix: "plan-complete" }));
                }
            ...
            {streamState.isStreaming && hasStreamingContent && streamState.streamingMessage && (
              <MessageView message={streamState.streamingMessage as AgentMessage} isStreaming modelNames={modelNames} cwd={messageCwd} onOpenFile={onOpenFile} onOpenBrowser={onOpenBrowser} />
            )}
