...
import type { BuiltinSlashCommandResult, CompactResultInfo, QueuedMessages, SlashCommandInfo, AgentMode } from "@/hooks/useAgentSession";
import type { SkillsResponse } from "@/lib/api-types";
import type { ExtensionUiRequest, TextContent, UserMessage } from "@/lib/types";
import {
  clearDraft,
  getDraft,
...
import { ContextProgressRing } from "./ContextProgressRing";
...
interface Props {
  ...
  cwd?: string | null;
  /** True when pi-plan-mode has a ready plan awaiting the user's decision. */
  planReady?: boolean;
  /** True while pi-plan-mode is active (drives the input border highlight). */
  planModeActive?: boolean;
  /** Quick actions shown when a plan is ready or saved but no menu request is
   *  cached: implement / show / save / export / exit (driven by /plan
   *  subcommands). */
  onPlanAction?: (action: "implement" | "show" | "save" | "export" | "exit") => void;
  /** True when a plan is saved for later (pi-plan-mode status "plan saved");
   *  keeps the inline bar available with saved-plan actions. */
  planSaved?: boolean;
  /** Cached pi-plan-mode menu request (ready / saved / active implementation)
   *  rendered inline as buttons; consumed by clicking one of them. */
  planModeMenu?: ExtensionUiRequest | null;
  /** Respond to the cached menu inline (value = chosen option string). */
  onPlanMenuRespond?: (option: string) => void;
  /** Live context usage from the SDK — drives the center progress ring. */
  contextUsage?: ContextUsage | null;
  /** Session token totals — breakdown rows inside the ring popover. */
...
// Map pi-plan-mode menu option strings to short Chinese labels. Saved/active
// menus reuse the ready menu's wording for implement/fresh/export and add
// clear/start-new entries — "clear" must be checked before "save" because
// "Clear saved plan" contains "save".
function planMenuLabel(option: string): string {
  const o = option.toLowerCase();
  if (o.includes("implement here")) return "▶ 开始实施";
  if (o.includes("start fresh")) return "✦ 新会话实施";
  if (o.includes("new plan")) return "✦ 开始新方案";
  if (o.includes("export")) return "⤓ 导出方案";
  if (o.includes("clear")) return "✕ 清除方案";
  if (o.includes("save")) return "⭳ 保存备用";
  if (o.includes("stay")) return "✎ 继续规划";
  if (o.includes("discard") || o.includes("exit")) return "✕ 退出";
  return option;
}
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput({
  ...
  draftKey,
  cwd,
  planReady, planSaved, planModeActive, onPlanAction, planModeMenu, onPlanMenuRespond,
  ...
  const [planBarDismissed, setPlanBarDismissed] = useState(false);
  ...
  const promptOptimizeErrorTimer = useRef<number | null>(null);

  useEffect(() => () => {
    if (promptOptimizeErrorTimer.current !== null) window.clearTimeout(promptOptimizeErrorTimer.current);
  }, []);

  // One-shot LLM call through /api/prompt-optimize; replaces the composer text
  // in place (draft autosave follows value). Independent of any AgentSession.
  const handleOptimizePrompt = useCallback(async () => {
    const current = valueRef.current;
    if (promptOptimizing || bashMode || !model || !current.trim()) return;
    ...
    try {
      const res = await fetch("/api/prompt-optimize", {
        ...
        body: JSON.stringify({ prompt: current, provider: model.provider, modelId: model.modelId }),
      });
      const data = await res.json() as { ok: boolean; optimizedPrompt?: string; error?: string };
      if (!res.ok || !data.ok || typeof data.optimizedPrompt !== "string" || !data.optimizedPrompt.trim()) {
        throw new Error(data.error || `Request failed (${res.status})`);
      }
      const optimized = data.optimizedPrompt.trim();
      const ta = textareaRef.current;
      valueRef.current = optimized;
      setValue(optimized);
      ...
      setPromptOptimizeError(err instanceof Error ? err.message : String(err));
      if (promptOptimizeErrorTimer.current !== null) window.clearTimeout(promptOptimizeErrorTimer.current);
      promptOptimizeErrorTimer.current = window.setTimeout(() => {
        promptOptimizeErrorTimer.current = null;
        setPromptOptimizeError(null);
      }, 3000);
    } finally {
      setPromptOptimizing(false);
    }
  }, [bashMode, model, promptOptimizing]);

  const canOptimizePrompt = !promptOptimizing && !bashMode && !!model && !!value.trim();
  ...
  // Mode buttons write a /plan or /goal prefix into the input box (just like
  // bash mode's "!"). The user then types the rest and sends — the existing
  // slash-command path forwards it to the extension. inputMode only mirrors the
  // current prefix so the active button lights up.
  const inputMode: AgentMode =
    trimmedValue.startsWith("/plan ") || trimmedValue === "/plan" ? "plan" :
    trimmedValue.startsWith("/goal ") || trimmedValue === "/goal" ? "goal" : "auto";
  const applyModePrefix = useCallback((mode: AgentMode) => {
    const stripped = valueRef.current.replace(/^\/(plan|goal)(\s+|$)/, "");
    const next = mode === "auto" ? stripped : `/${mode} ${stripped}`;
  ...
  // Re-show the plan bar each time a new plan becomes ready or saved, or a new
  // plan-mode menu request arrives (ready / saved / active implementation).
  // Menu clicks dismiss the bar immediately; a fresh menu request or state
  // transition brings it back.
  useEffect(() => {
    if (planReady || planSaved || planModeMenu) setPlanBarDismissed(false);
  }, [planReady, planSaved, planModeMenu]);

  // Bar title reflects which plan-mode state/menu is showing: ready (default),
  // saved plan, or active implementation — detected from distinctive options
  // when a menu is cached, else from the planSaved status flag.
  const planBarTitleKey = (() => {
    const opts = ((planModeMenu as { options?: string[] } | null)?.options ?? []).map((v) => v.toLowerCase());
    if (opts.some((v) => v.includes("clear saved plan"))) return "chat.planSavedBar" as const;
    if (opts.some((v) => v.includes("clear active implementation plan"))) return "chat.planActiveBar" as const;
    if (planSaved) return "chat.planSavedBar" as const;
    return "chat.planReady" as const;
  })();
    ...
    >
      ...
      <div style={{ maxWidth: 820, margin: "0 auto" }}>
        ...
        <div style={{ position: "relative", minWidth: 0 }}>
          ...
          {/* Plan-ready actions, rendered inline from pi-plan-mode's ready menu
              (implement / start fresh / export / save / stay / exit). The menu
              buttons (planModeMenu) respond via respondToPlanMenu with no
              running gate, so they must NOT be gated on isStreaming — pi-plan-mode
              holds the prompt pending while its ready-menu awaits a response, so
              sessionBusy (isStreaming) stays true; hiding the menu there would
              deadlock the approval flow. The fixed fallback buttons (planReady &&
              onPlanAction) go through handlePlanAction, which early-returns while
              the agent is running — showing them mid-run would swallow the first
              click (the "click twice" bug), so they are only rendered when idle. */}
          {(planModeMenu || planSaved || (planReady && !isStreaming && onPlanAction)) && !planBarDismissed && (
            <div style={{
              ...
              <span style={{ fontWeight: 600, color: "rgba(180,130,0,1)", display: "flex", alignItems: "center", gap: 4, whiteSpace: "nowrap" }}>
                <span>▣</span> {t(planBarTitleKey)}
                {planSaved && !planModeMenu && (
                  <span style={{ fontWeight: 400, color: "var(--text-dim)", fontSize: 11 }}>· {t("chat.planSavedHint")}</span>
                )}
              </span>
              <div style={{ flex: 1, minWidth: 0 }} />
              {planModeMenu && onPlanMenuRespond
                ? ((planModeMenu as { options?: string[] }).options ?? [])
                    // pi-plan-mode options that are redundant in the inline bar:
                    // - "Show latest proposed plan": plan already shown as 实施方案 card above
                    // - "Show saved plan" / "Show active implementation plan": same, plan is visible
                    // - "Settings" / "How Plan mode works" / "Choose tools, then start…": settings
                    //   UX belongs in the Settings dialog, not this bar
                    // - "Start Plan mode": launch is handled by typing /plan start
                    .filter((opt) => !/^show latest proposed plan$|^show saved plan$|^show active implementation plan$|^settings$|^how plan mode works$|^choose tools, then start\.\.\.$|^start plan mode$/i.test(opt))
                    .map((opt) => {
                    const isPrimary = /implement here/i.test(opt);
                    return (
                      <button key={opt} type="button" onClick={() => {
                        // Saving/exporting keep the bar visible: saving
                        // transitions it into the "saved plan" bar, and an
                        // export leaves the plan state untouched. Other actions
                        // dismiss it immediately.
                        if (!/save for later|export plan/i.test(opt)) setPlanBarDismissed(true);
                        onPlanMenuRespond(opt);
                      }}
                        style={{
                          padding: "4px 12px",
                          background: isPrimary ? "rgba(245,158,11,0.15)" : "none",
                          border: isPrimary ? "1px solid rgba(245,158,11,0.4)" : "1px solid var(--border)",
                          borderRadius: 7,
                          color: isPrimary ? "rgba(180,130,0,1)" : "var(--text-muted)",
                          cursor: "pointer", fontSize: 12, fontWeight: isPrimary ? 600 : 500,
                          whiteSpace: "nowrap",
                        }}>
                        {planMenuLabel(opt)}
                      </button>
                ...
                : planSaved ? (
                  <>
                    <button type="button" onClick={() => { setPlanBarDismissed(true); onPlanAction?.("implement"); }}
                      style={{ padding: "4px 12px", background: "rgba(245,158,11,0.15)", border: "1px solid rgba(245,158,11,0.4)", borderRadius: 7, color: "rgba(180,130,0,1)", cursor: "pointer", fontSize: 12, fontWeight: 600, whiteSpace: "nowrap" }}>
                      ▶ {t("chat.planImplement")}
                    </button>
                    <button type="button" onClick={() => onPlanAction?.("show")}
                      style={{ padding: "4px 10px", background: "none", border: "1px solid var(--border)", borderRadius: 7, color: "var(--text-muted)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ▤ {t("chat.planShow")}
                    </button>
                    <button type="button" onClick={() => onPlanAction?.("export")}
                      style={{ padding: "4px 10px", background: "none", border: "1px solid var(--border)", borderRadius: 7, color: "var(--text-muted)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ⤓ {t("chat.planExport")}
                    </button>
                    <button type="button" onClick={() => { setPlanBarDismissed(true); onPlanAction?.("exit"); }}
                      style={{ padding: "4px 10px", background: "none", border: "none", color: "var(--text-dim)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ✕ {t("chat.planClear")}
                    </button>
                  </>
                ) : (
                  <>
                    ...
                    <button type="button" onClick={() => { setPlanBarDismissed(true); onPlanAction?.("save"); }}
                      style={{ padding: "4px 10px", background: "none", border: "1px solid var(--border)", borderRadius: 7, color: "var(--text-muted)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ⭳ {t("chat.planSave")}
                    </button>
                    <button type="button" onClick={() => { setPlanBarDismissed(true); onPlanAction?.("export"); }}
                      style={{ padding: "4px 10px", background: "none", border: "1px solid var(--border)", borderRadius: 7, color: "var(--text-muted)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ⤓ {t("chat.planExport")}
                    </button>
                    <button type="button" onClick={() => { setPlanBarDismissed(true); textareaRef.current?.focus(); }}
                      style={{ padding: "4px 10px", background: "none", border: "1px solid var(--border)", borderRadius: 7, color: "var(--text-muted)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ✎ {t("chat.planKeepPlanning")}
                    </button>
                    <button type="button" onClick={() => { setPlanBarDismissed(true); onPlanAction?.("exit"); }}
                      style={{ padding: "4px 10px", background: "none", border: "none", color: "var(--text-dim)", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap" }}>
                      ✕ {t("chat.planExit")}
          ...
          <div
            style={{
              ...
              alignItems: "stretch",
              background: "var(--bg)",
              border: `1px solid ${bashMode ? "var(--tool-bg)" : planModeActive ? "rgba(245,158,11,0.45)" : isStreaming && (onSteer || onFollowUp)
                ? "rgba(234,179,8,0.4)"
          ...
          <textarea
            ...
            placeholder={
              isStreaming && (onSteer || onFollowUp)
                ...
                : planModeActive ? t("chat.planPlaceholder")
          ...
          {/* Bottom row: mode selector (left) ... send/steer (right) */}
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            {!isStreaming && (
              <div style={{ display: "flex", alignItems: "center", gap: 2, flexShrink: 0, background: "var(--bg-panel)", border: "1px solid color-mix(in srgb, var(--border) 65%, transparent)", borderRadius: 8, padding: 2 }}>
                {([
                  { mode: "auto" as const, label: t("chat.modeAuto"), icon: "⚙", desc: t("chat.modeAutoDesc") },
                  { mode: "plan" as const, label: t("chat.modePlan"), icon: "▣", desc: t("chat.modePlanDesc") },
                  { mode: "goal" as const, label: t("chat.modeGoal"), icon: "🎯", desc: t("chat.modeGoalDesc") },
                ]).map((opt) => {
                  const active = inputMode === opt.mode;
                  const accent = opt.mode === "plan" ? "rgba(180,130,0,1)" : opt.mode === "goal" ? "rgba(99,102,241,1)" : "var(--text)";
                  const soft = opt.mode === "plan" ? "rgba(245,158,11,0.14)" : opt.mode === "goal" ? "rgba(99,102,241,0.14)" : "var(--bg-hover)";
                  return (
                    <button
                      key={opt.mode}
                      type="button"
                      onClick={() => applyModePrefix(opt.mode)}
                      title={opt.desc}
                      style={{
                        display: "flex", alignItems: "center", gap: 4,
                        padding: "3px 9px", height: 24,
                        background: active ? soft : "none",
        ...
        <div style={{
          ...
          <div ref={controlsMenuRef} style={{
            ...
            <button
              ...
              title={promptOptimizeError ?? (promptOptimizing ? t("chat.optimizing") : t("chat.optimizePrompt"))}
              ...
              style={{
                ...
                border: "none",
                borderRadius: 9,
                color: promptOptimizeError ? "#ef4444" : "var(--text-muted)",
                cursor: !canOptimizePrompt ? "not-allowed" : "pointer",
                fontSize: 12,
                opacity: !canOptimizePrompt ? 0.5 : 1,
                transition: "background 0.12s, color 0.12s",
              }}
              onMouseEnter={(e) => {
                if (!canOptimizePrompt) return;
                e.currentTarget.style.background = "var(--bg-hover)";
                if (!promptOptimizeError) e.currentTarget.style.color = "var(--text)";
              }}
              onMouseLeave={(e) => {
                e.currentTarget.style.background = "none";
                e.currentTarget.style.color = promptOptimizeError ? "#ef4444" : "var(--text-muted)";
              }}
            >
              {promptOptimizing ? (
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" style={{ animation: "spin 0.8s linear infinite", flexShrink: 0 }} aria-hidden="true">
              ...
              {(!isMobile || controlsMenuOpen) && <span style={{ whiteSpace: "nowrap" }}>{t("chat.optimize")}</span>}
            </button>
            <ContextProgressRing contextUsage={contextUsage ?? null} sessionStats={sessionStats ?? null} />
              ...
              <div ref={toolDropdownRef} style={{ position: "relative" }}>
                <button
                  ...
                  disabled={isStreaming || planModeActive}
                   title={planModeActive ? t("chat.toolsLockedByMode") : t("chat.changeToolPreset") + `: ${toolPresetLabel}`}
