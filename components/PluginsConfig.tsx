...
function installLocation(scope: PluginScope, cwd: string): string {
  return scope === "project"
    ? `${shortenPath(cwd)}/.pi-studio/{npm,git}`
    : "~/.pi-studio/{npm,git}";
}
...
function AddPluginPanel({
  ...
  const examples = ["npm:pi-web-access", "npm:pi-lens", "npm:pi-simplify", "npm:@narumitw/pi-goal"];
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 680 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12, minWidth: 0, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {/* Built-ins are version-pinned by the pi-studio release (see
              lib/builtin-extension-sources.ts): "update" would reinstall the
              same pinned spec — pointless button, and a confusing "why is it
              still 0.9.6" moment. Upgrades ship with pi-studio itself. */}
          {!pkg.builtin && (
            <button
...
export function PluginsConfig({
  ...
  const loadPlugins = useCallback(async () => {
    ...
    setError(null);
    try {
      // 403（allow-list 竞态：validate 尚未落地 / 服务器重启后内存列表丢失）
      // 自动重试一次，避免“权限不足”错误一直挂在面板上。
      const next = await fetchJsonWithRetry<PluginsResponse>(
      ...
      >
        ...
        {data?.seedStatus &&
          (data.seedStatus.seeding ||
            data.seedStatus.results.some((r) => r.action === "failed")) && (
            <div
              role="status"
              style={{
                padding: "8px 18px",
                borderBottom: "1px solid var(--border)",
                background: "var(--bg-panel)",
                color: data.seedStatus.seeding ? "var(--text-muted)" : "#ef4444",
                fontSize: 12,
              }}
            >
              {data.seedStatus.seeding
                ? t("plugin.seeding")
                : `${t("plugin.seedFailed")}: ${data.seedStatus.results
