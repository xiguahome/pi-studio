...
import { useHighlighter } from "./LazySyntaxHighlighter";
...
export const CodeBlock = memo(function CodeBlock({ code, lang, headerAction, isStreaming }: CodeBlockProps) {
  ...
  // Lazy-load react-syntax-highlighter only when a non-streaming block is
  // first rendered. While loading (or streaming), fall back to a plain <pre>
  // with the code content — visually identical to the streaming fallback.
  const highlighter = useHighlighter(!!isStreaming);
  ...
  return (
    <div className="markdown-code-block">
      ...
      {isStreaming || !highlighter ? (
        ...
        <highlighter.Prism
          language={lang || "text"}
          style={isDark ? highlighter.vscDarkPlus : highlighter.vs}
          showLineNumbers
