...
- npm package: `@agegr/pi-studio`
- GitHub Release: `agegr/pi-studio`
...
- npm is authenticated as an account that can publish `@agegr/pi-studio`.
...
- If `npm view @agegr/pi-studio version` briefly shows the previous version, check the exact version instead:

```bash
npm view @agegr/pi-studio@<version> version --registry https://registry.npmjs.org/
npm view @agegr/pi-studio versions --json --registry https://registry.npmjs.org/
```
...
gh release view v<version> --repo agegr/pi-studio
```
...
- 发布 npm 包 `@agegr/pi-studio@<version>`。
...
- Published npm package `@agegr/pi-studio@<version>`.
```
...
```bash
gh release edit v<version> --repo agegr/pi-studio --notes-file - <<'EOF'
## 中文
